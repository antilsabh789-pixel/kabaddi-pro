...
export default function FindTeamsScreen({ onBack }: FindTeamsScreenProps) {
  ...
  // Request geolocation — non-blocking: grounds list still loads without GPS
  ...
  // Fetch grounds — works with or without geolocation
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const groundsRes = await fetch(`/api/nearby-grounds?${params}`);
        if (groundsRes.ok) {
  ...
  const itemLabel = grounds.length === 1 ? '1 ground found' : `${grounds.length} grounds found`;
            ...
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
              ...
              <p className="text-warm-400 dark:text-warm-500 text-xs mt-1">
                {radiusFilter === 'everywhere'
                  ? (language === 'hi' ? 'अभी कोई ग्राउंड उपलब्ध नहीं है' : 'No grounds available right now')
                    ...
                    <Card className={`bg-white/80 dark:bg-warm-800/50 border-warm-200/50 dark:border-warm-700/50 hover:shadow-md transition-shadow border-l-4 ${surfaceConfig ? `border-l-[${surfaceConfig.color}]` : 'border-l-warm-300 dark:border-l-warm-600'}`}>
                      <CardContent className="p-4">
                        <div className="flex items-start gap-3">
                          ...
                          {/* Distance — only show when we actually have a real distance */}
