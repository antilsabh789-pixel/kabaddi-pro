...
export default function GroundsScreen({ onClose, onSelect }: { onClose: () => void; onSelect?: (groundId: string, groundName: string) => void }) {
  ...
  }, [search, surfaceFilter, amenityFilter, sortOption, userLat, userLng]);
  ...
  // ─── Ground Detail View ─────────────────────────────────────────
  function GroundDetailView({ ground }: { ground: Ground }) {
            ...
            <div>
              ...
              <div className="grid grid-cols-2 gap-2">
                {Object.entries(AMENITY_CONFIG).map(([key, config]) => {
                  const formKey = `has${key.charAt(0).toUpperCase()}${key.slice(1).replace(/_([a-z])/g, (_, l) => l.toUpperCase())}` as keyof typeof form;
  ...
  return (
    ...
    >
      <div className="min-h-screen flex flex-col max-w-lg mx-auto">
        ...
        <AnimatePresence mode="wait">
          {selectedGround ? (
            <GroundDetailView key="detail" ground={selectedGround} />
          ) : (
