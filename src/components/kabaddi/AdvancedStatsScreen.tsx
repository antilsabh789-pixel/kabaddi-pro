...
import {
  ...
  Trophy, Target, Activity, TrendingUp,
} from 'lucide-react';
...
function CircularGauge({
  ...
  return (
    <div className="flex flex-col items-center gap-1">
      ...
      {sublabel && (
        <span className="text-[10px] text-warm-500 font-medium">{sublabel}</span>
      )}
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <svg width="280" height="160" viewBox="0 0 280 160">
        ...
        {/* Hot zones - Raider: highlight opponent half */}
        {(isRaider || isAllRounder) && (
          <>
            <motion.rect
              x="142" y="6" width="132" height="148" rx="4"
            ...
            />
            {/* Hot spots in raid zone */}
            <motion.circle cx="210" cy="55" r="18" fill="rgba(220, 38, 38, 0.25)"
              initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.7, type: 'spring', damping: 12 }} />
            <motion.circle cx="190" cy="100" r="14" fill="rgba(220, 38, 38, 0.2)"
              initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.8, type: 'spring', damping: 12 }} />
            <motion.circle cx="240" cy="80" r="12" fill="rgba(220, 38, 38, 0.18)"
              initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.9, type: 'spring', damping: 12 }} />
          </>
        )}

        {/* Hot zones - Defender: highlight own half corners */}
        {(isDefender || isAllRounder) && (
          <>
            <motion.rect
              x="6" y="6" width="132" height="148" rx="4"
            ...
            />
            {/* Corner hot spots */}
            <motion.circle cx="35" cy="30" r="16" fill="rgba(30, 41, 59, 0.2)"
              initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.7, type: 'spring', damping: 12 }} />
            <motion.circle cx="35" cy="130" r="16" fill="rgba(30, 41, 59, 0.2)"
              initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.8, type: 'spring', damping: 12 }} />
            <motion.circle cx="70" cy="80" r="12" fill="rgba(30, 41, 59, 0.15)"
              initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ delay: 0.9, type: 'spring', damping: 12 }} />
          </>
        )}
      </svg>
...
function generateMatchHistory(
  ...
  for (let i = 0; i < Math.min(7, matchCount); i++) {
...
export default function AdvancedStatsScreen({ userId, onClose }: AdvancedStatsScreenProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Handle API response format: { player, profile }
  ...
  const derivedStats = useMemo(() => {
    ...
    const matchHistory = generateMatchHistory(p, user!);
      ...
      >
        <header className="sticky top-0 z-10 bg-warm-50/90 backdrop-blur-md border-b border-warm-200/60">
          <div className="px-4 py-3 flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-full bg-warm-200 flex items-center justify-center text-warm-600"
            >
              <X className="w-4 h-4" />
            </button>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div key={i} className="h-40 rounded-2xl bg-warm-100 animate-pulse" />
          ))}
        </div>
      </motion.div>
    );
  ...
  if (!profile || !derivedStats) {
    return (
      ...
      >
        ...
        <button
          onClick={onClose}
          className="px-4 py-2 rounded-xl bg-brand-teal text-white font-bold text-sm"
        >
          Go Back
        </button>
      </motion.div>
          ...
          <motion.div variants={itemVariants}>
            <Card className="p-5 border-warm-200/60">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-black tracking-wider text-warm-800">
                  PERFORMANCE OVERVIEW
                </h2>
              </div>
              ...
              <div className="flex items-center gap-6">
                ...
                <div className="flex-1 space-y-3">
                  <div>
                    ...
                    <p className="text-xl font-black text-warm-800">{derivedStats.totalPoints}</p>
                  </div>
                  <div className="flex items-center gap-4">
                    <div>
                      <p className="text-[10px] text-warm-500 font-semibold uppercase tracking-wide">Matches</p>
                      <p className="text-lg font-bold text-warm-700">{derivedStats.matchCount}</p>
                    </div>
                    <div>
                      <p className="text-[10px] text-warm-500 font-semibold uppercase tracking-wide">Avg Pts/Match</p>
                      <p className="text-lg font-bold text-warm-700">{derivedStats.avgPoints.toFixed(1)}</p>
                    </div>
                  </div>
                </div>
              </div>
            ...
            <PremiumLock feature="Raid Analysis">
              <Card className="p-5 border-warm-200/60">
                ...
                <div className="flex items-start gap-5">
                  {/* Raid success gauge */}
                  <CircularGauge
                    value={derivedStats.raidSuccessRate}
                    max={100}
                    size={90}
                    strokeWidth={7}
                    color="#DC2626"
                  ...
                  >
                    <span className="text-base font-black text-brand-red">
                      {derivedStats.raidSuccessRate.toFixed(1)}%
                    </span>
                    <span className="text-[8px] font-semibold text-warm-500 uppercase tracking-wide">
                      Success
                    </span>
                  </CircularGauge>

                  <div className="flex-1 space-y-3">
                    {/* Raid trend (last 5 matches) */}
                    <div>
                      <p className="text-[10px] text-warm-500 font-semibold uppercase tracking-wide mb-1.5">
                        Raid Pts/Match Trend
                      </p>
                      ...
                      <div className="flex gap-1.5 mt-0.5">
                        {['M1', 'M2', 'M3', 'M4', 'M5'].map((label) => (
                          <span key={label} className="flex-1 text-center text-[8px] text-warm-400 font-medium">
                            {label}
                          </span>
                        ))}
                      </div>
                    ...
                    {/* Successful vs Failed Raids bar */}
                    <div>
                      <div className="flex items-center justify-between text-[10px] mb-1">
                        <span className="text-warm-500 font-semibold uppercase tracking-wide">
                          Raids Breakdown
                        </span>
                        <span className="text-warm-400">
                          {profile.successfulRaids}S / {derivedStats.failedRaids}F
                        </span>
                      </div>
                      <div className="h-3 bg-warm-200 rounded-full overflow-hidden flex">
                        <motion.div
                          ...
                          initial={{ width: 0 }}
                          animate={{
                            width: profile.totalRaids > 0
                              ? `${(profile.successfulRaids / profile.totalRaids) * 100}%`
                              : '0%',
                          }}
                        ...
                        <motion.div
                          ...
                          animate={{
                            width: profile.totalRaids > 0
                              ? `${(derivedStats.failedRaids / profile.totalRaids) * 100}%`
                              : '0%',
                          }}
                      ...
                      <div className="flex items-center gap-3 mt-1">
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-[9px] text-warm-500">Successful</span>
                        </div>
                        <div className="flex items-center gap-1">
                          <div className="w-2 h-2 rounded-full bg-warm-300" />
                          <span className="text-[9px] text-warm-500">Failed</span>
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <motion.div variants={itemVariants}>
            <PremiumLock feature="Defense Analysis">
              <Card className="p-5 border-warm-200/60">
                ...
                <div className="flex items-start gap-5">
                  {/* Tackle success gauge */}
                  <CircularGauge
                    value={derivedStats.tackleSuccessRate}
                    max={100}
                    size={90}
                    strokeWidth={7}
                    color="#1E293B"
                  ...
                  >
                    <span className="text-base font-black text-brand-navy">
                      {derivedStats.tackleSuccessRate.toFixed(1)}%
                    </span>
                    <span className="text-[8px] font-semibold text-warm-500 uppercase tracking-wide">
                      Success
                    </span>
                  ...
                  <div className="flex-1 space-y-3">
                    ...
                    <div className="flex items-center gap-3 p-2.5 rounded-xl bg-brand-gold/10 border border-brand-gold/20">
                      ...
                      <div>
                        <p className="text-[10px] text-warm-500 font-semibold uppercase tracking-wide">
                          Super Tackles
                        </p>
                        <p className="text-lg font-black text-brand-gold-dark">{profile.superTackles}</p>
                      </div>
                    </div>

                    {/* Successful vs Failed Tackles bar */}
                    <div>
                      <div className="flex items-center justify-between text-[10px] mb-1">
                        <span className="text-warm-500 font-semibold uppercase tracking-wide">
                          Tackles Breakdown
                        ...
                        <span className="text-warm-400">
                          {profile.successfulTackles}S / {derivedStats.failedTackles}F
                        </span>
                      </div>
                      <div className="h-3 bg-warm-200 rounded-full overflow-hidden flex">
                        <motion.div
                          ...
                          initial={{ width: 0 }}
                          animate={{
                            width: profile.totalTackles > 0
                              ? `${(profile.successfulTackles / profile.totalTackles) * 100}%`
                              : '0%',
                          }}
                          transition={{ duration: 0.8, ease: 'easeOut', delay: 0.5 }}
                        />
                        <motion.div
                          ...
                          animate={{
                            width: profile.totalTackles > 0
                              ? `${(derivedStats.failedTackles / profile.totalTackles) * 100}%`
                              : '0%',
                          }}
                          transition={{ duration: 0.8, ease: 'easeOut', delay: 0.6 }}
                        />
